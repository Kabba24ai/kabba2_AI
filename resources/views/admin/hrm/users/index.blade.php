    ...
    <div class="">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex flex-wrap justify-right items-center gap-2">
                <a href="{{ route('admin.hrm.roles.manage') }}"
                    class="bg-gray-700 text-white px-4 py-2 rounded flex items-center gap-2 text-sm font-medium">
                    <x-heroicon-o-shield-check class="w-5 h-5" />
                    Manage Roles
                </a>
                <a href="{{ route('admin.hrm.users.create') }}"
                    class="bg-blue-600 text-white px-4 py-2 rounded flex items-center gap-2 text-sm font-medium">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mt-6">
            ...
            <div class="bg-white border border-gray-200 rounded-md p-4 w-full">
                <div class="flex justify-left">
                    <ul class="flex flex-wrap gap-2 text-sm sm:text-base font-medium">
                        ...
                        <li id="filter__Roles" data-category="all"
                            class="cursor-pointer text-sm px-4 py-2 bg-blue-600 text-white  rounded ">All Roles</li>

                        @foreach ($roles as $role)
                            <li id="filter__{{ $role->short_name }}" data-category="{{ $role->short_name }}"
        ...
        <div class="bg-white border rounded-md p-4 w-full mt-6">
            <div class="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 w-full">
                ...
                <!-- Search Field -->
                <div
                    class="flex items-center w-full sm:w-full border rounded-md px-3 py-2 text-sm shadow-sm border-gray-300">
                    ...
                    <input type="text" value="{{ request('search_name') }}" name="search_name"
                ...
                <div class="w-full sm:w-48 relative">
                    ...
                    <div
                        class="flex items-center border border-gray-300 rounded-md shadow-sm px-3 py-2 bg-white text-sm text-gray-700">
                        <!-- Filter Icon -->
                        <x-heroicon-o-funnel class="w-5 h-5 text-gray-500" />
                        <!-- Dropdown -->
                        <select id="user_status" name="user_status"
                            class="w-full bg-transparent outline-none text-gray-700 pr-4">
...
@push('js')
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const screenKey = "user_filters";

            const fieldMap = {
                search_name: nameInput,
                user_status: statusSelect
            };
