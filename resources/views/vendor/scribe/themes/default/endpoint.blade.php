...
@endforeach </code></pre></details> @endif
        @php($responseElId = 'example-response-'.$endpoint->endpointId().'-'.$loop->index)
        <div class="scribe-response-wrapper">
        <button type="button" class="scribe-copy-btn" data-copy-target="{{ $responseElId }}">Copy</button>
        <pre>
@if($response->isBinary())
<code id="{{ $responseElId }}">{!! u::trans("scribe::endpoint.responses.binary") !!} - {{ htmlentities(str_replace("<<binary>>", "", $response->content)) }}</code>
@elseif($response->status == 204)
<code id="{{ $responseElId }}">{!! u::trans("scribe::endpoint.responses.empty") !!}</code>
@else
@php($parsed = json_decode($response->content))
{{-- If response is a JSON string, prettify it. Otherwise, just print it --}}
<code id="{{ $responseElId }}" class="language-json" style="max-height: 300px;">{!! htmlentities($parsed != null ? json_encode($parsed, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES) : $response->content) !!}</code>
@endif </pre>
        </div>
    @endforeach
@endif
...
<span id="execution-results-{{ $endpoint->endpointId() }}" hidden>
    ...
    <div class="scribe-response-wrapper">
    <button type="button" class="scribe-copy-btn" data-copy-target="execution-response-content-{{ $endpoint->endpointId() }}">Copy</button>
