...
/**
 * Canonical Schedule Conflict aggregation. This is the single source of truth
 * for every conflict category and for the "All" total, consumed by BOTH the
 * Schedule Conflicts page (which renders the datasets) and the Dashboard
 * Schedule Conflicts card (which shows the total). Keeping one implementation
 * guarantees the two surfaces can never disagree.
 *
 * The "All" total is the arithmetic sum of the six category counts — the
 * established page semantics (mixed units, no cross-category dedup). This
 * service preserves that exactly; it does not change conflict definitions.
 */
class ScheduleConflictService
{
     ...
     */
    public const INVENTORY_LOCATION_LOOKAHEAD_DAYS = 14;

    /**
     * Build every conflict category under one definition.
     *
     * @param  array{search?:string,category?:string|int,store?:string|int,section?:string}  $filters
     *         Page filters. Omit (or pass []) for the unscoped board — this is
     *         what the Dashboard uses so its total matches the page's default "All".
     * @return array{
     *   doubleBookings:array, backToBackAlerts:array, damagedBookings:array,
     *   overdueEquipmentConflicts:array, noDirectAssignmentGroups:array,
     *   inventoryLocationConflicts:array, total:int, counts:array<string,int>
     * }
     */
    public function build(array $filters = []): array
    {
        $search   = trim((string) ($filters['search'] ?? ''));
        $category = $filters['category'] ?? '';   // product_category_id on equipment
        $store    = $filters['store'] ?? '';        // store_id on equipment
        $section  = $filters['section'] ?? '';      // which conflict type to show
        ...
        // ── Totals ────────────────────────────────────────────────────────────
        $counts = [
            'double_bookings'      => count($doubleBookings),
            'back_to_back'         => count($backToBackAlerts),
            'damaged'              => count($damagedBookings),
            'overdue'              => count($overdueEquipmentConflicts),
            'no_direct_assignment' => count($noDirectAssignmentGroups),
            'inventory_location'   => count($inventoryLocationConflicts),
        ];
        $total = array_sum($counts);

        return [
            'doubleBookings'             => $doubleBookings,
            'backToBackAlerts'           => $backToBackAlerts,
            'damagedBookings'            => $damagedBookings,
            'overdueEquipmentConflicts'  => $overdueEquipmentConflicts,
            'noDirectAssignmentGroups'   => $noDirectAssignmentGroups,
            'inventoryLocationConflicts' => $inventoryLocationConflicts,
            'counts'                     => $counts,
            'total'                      => $total,
        ];
    }

    /** Just the "All" total — the number the Dashboard card shows. */
    public function total(array $filters = []): int
    {
        return $this->build($filters)['total'];
    }
}
