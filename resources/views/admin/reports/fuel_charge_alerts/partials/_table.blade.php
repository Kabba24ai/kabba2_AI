...
@php
    $crmFuelRecords = $crmFuelRecords ?? collect();
    $hasAny = $records->isNotEmpty() || $crmFuelRecords->isNotEmpty();
@endphp
...
@else
    <div class="space-y-3">
        ...
        {{-- Rental-checklist fuel charges (order_products) --}}
        ...
        {{-- Manually-added fuel charges (customer_accounts / CRM / Dashboard) --}}
        @foreach($crmFuelRecords as $crm)
            @php
                ...
                $crmTotal    = ($crm->sales_tax_type === 'add' && $crmTaxRate > 0)
                    ...
                    : $crmBase;
                $crmBadge    = $fuelStatusBadge($crm->fuel_alert_status);
            @endphp
