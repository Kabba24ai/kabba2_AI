...
<script>
    ...
    function rebuildEquipmentDropdown(preselectId) {
        ...
        filtered.forEach(e => {
            ...
            if (preselectId && preselectId === e.id) opt.selected = true;
    ...
    // Pre-select category to match the saved equipment
    if (savedEquipId) {
        const savedEq = allEquipment.find(e => e.id === savedEquipId);
        if (savedEq && savedEq.category_id) {
            categoryFilter.value = savedEq.category_id;
        }
    }
    ...
    rebuildEquipmentDropdown(savedEquipId);
</script>
@endpush
