...
| D2 | Server-compute admin-web Rental Ready status? | Client (explicit instruction: "Use observability-first behavior for the admin Rental Ready workflow") | 2026-07-11 | Approved recommended option — **observe first, enforce later**. `RentalReadyCompletionCalculator` (PR-B2 Stage 2) is called from the admin-web `StoreController` (PR-B2 Stage 3) in parallel with the existing client-trusted write; a structured `api_errors` warning is logged whenever the computed result disagrees with the submitted status/counts, but the persisted values remain exactly what the client submitted — no enforcement yet. Enforcement (switching admin-web to fully server-computed status) remains an explicit, separately-gated future decision pending review of this telemetry. Implementation reference: `PR-B2_STAGE3_ADMIN_OBSERVABILITY.md`, `PR-B2_RENTAL_READY_CALCULATOR.md` §5 |
| D3 | Re-enable required-questions guard? | Engineering investigation (git archaeology), under Phase 2 approval | 2026-07-09 | Approved recommended option — **observe, do not enforce**. Git history showed the guard (`RentalReadyChecklists\SaveController.php`) was written already-disabled at the endpoint's original commit, with no evidence it was ever live or that its condition is wrong. Staged as an `api_errors` observability warning (reusing the PR-A4 pattern) — the request still succeeds even when questions are unanswered. Re-enabling as a hard 422 rejection remains a future decision, to be informed by this telemetry. Implementation reference: `PR-B3_VALIDATION_GUARDS.md` |
| D4 | Re-enable invalid-equipment-status guard? | Engineering investigation (git archaeology), under Phase 2 approval | 2026-07-09 | Approved a **split outcome** across the guard's two independent halves, per the git-history investigation: the `isAvailable()` half was confirmed obsolete (contradicted by the endpoint's own downstream logic and by `SaveController.php`'s documented design) and was **permanently removed** — not staged, not logged, deleted outright. The `isRented()` half was kept and staged as an `api_errors` observability warning only (mirroring D3), since the sibling `SaveController.php` already enforces an equivalent live rule on the write path, making eventual re-enablement here plausible but not yet decided. Implementation reference: `PR-B3_VALIDATION_GUARDS.md` |
| D5 | Confirm no schema unification in Phase 2? | Client (explicit "no database migrations" / "no schema changes" instructions issued for each of PR-B4.1, PR-B4.2, PR-B4.3) | 2026-07-11 | **Confirmed as scoped — logic-sharing only, no schema change.** All three PR-B4 sub-PRs (Categories, Questions, Templates) shipped as shared *services* (`CategoryCrudService`, `QuestionCrudService`, `TemplateCrudService`) consolidating controller-level CRUD mechanics only; zero migrations were written, and every confirmed cross-tree asymmetry (Rental Ready soft-delete vs. Customer Admin hard-delete/cascade, the `equipment_category_id` FK asymmetry, etc.) was deliberately preserved, not unified. A full schema/model unification is explicitly carried forward as a Phase 3 candidate item (`ARCH-1` in `PHASE3_IMPLEMENTATION_PLAN.md`), requiring its own dedicated feasibility study and stakeholder go/no-go before any implementation planning. Implementation reference: `PR-B4_1_CATEGORY_REFACTOR.md`, `PR-B4_2_QUESTION_REFACTOR.md`, `PR-B4_3_TEMPLATE_REFACTOR.md` |
...
**Update (2026-07-14):** all five decisions (D1-D5) are now recorded above with decider, date, outcome, and implementation reference — retroactively corrected per `PHASE3_IMPLEMENTATION_PLAN.md` DOC-1, since D2-D5 were actually decided and implemented during Phase 2 but this log was never updated to reflect it at the time. No code was modified to make this correction; it is a documentation-accuracy fix only, describing engineering work that already shipped.
