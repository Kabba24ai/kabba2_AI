...
class ScheduleSection extends Component
{
    ...
    /**
     * The Dashboard conflict count is the same "All" total the Schedule
     * Conflicts page shows. It delegates to the canonical
     * ScheduleConflictService (no filters = the unscoped board) so the two
     * surfaces can never diverge — previously this was a duplicate query that
     * omitted Inventory Location conflicts and under-reported the total.
     */
    private function getScheduleConflictCount(): int
    {
        return app(\App\Services\Orders\ScheduleConflictService::class)->total();
    }
}
