...
namespace App\Http\Controllers\Admin\Tasks\Billing\FuelCharges;
 ...
 *
 * Relocated verbatim from Reports (was admin.reports.fuel-charge-workspace)
 * — same read-only shell, same canonical services, same endpoints. Only the
 * route name, URL, and view namespace changed; the operational behavior is
 * unchanged.
 *
 ...
 * dashboard/Billing Engine endpoints — no new business logic lives here.
 */
class IndexController extends Controller
{
    ...
    public function __invoke(Request $request)
    {
        ...
        if ($request->boolean('fragment')) {
            return response()->json([
                'success' => true,
                'queue_html' => view('admin.tasks.billing.partials._queue', ['alerts' => $alerts, 'chargeType' => 'fuel'])->render(),
        ...
        return view('admin.tasks.billing.fuel_charges.index', compact(
    ...
    private function terminalRecords(Request $request, string $status): Collection
    {
        ...
        // Billing Engine bridge lookup: terminal rows gain their canonical
        // BillingCharge reference so the shared action bar can offer
        // charge-keyed actions — most importantly Refund, which renders only
        // when the PAID bridge row still has a remaining refundable balance.
