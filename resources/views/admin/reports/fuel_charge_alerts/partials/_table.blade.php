...
@else
    <div class="space-y-3">
        @foreach($allFuelRecords as $item)
            @if($item['_source'] === 'op')
                {{-- Rental-checklist fuel charge (order_products) --}}
                @php
                    $record    = $item['_model'];
                    $order     = $record->order;
            ...
            @else
                {{-- Manually-added fuel charge (customer_accounts / CRM / Dashboard) --}}
                @php
                    $crm         = $item['_model'];
                ...
                <div class="border border-gray-200 rounded-xl bg-white p-4 hover:shadow-md transition">
                    <div class="flex justify-between gap-4">
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="flex items-center gap-2 flex-wrap">
                                <span class="text-sm font-medium text-gray-500">Customer:</span>
                                <h3 class="text-sm font-semibold text-gray-900">{{ $crmCustomer?->full_name ?? '—' }}</h3>
