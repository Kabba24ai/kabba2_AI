    ...
    <div class="flex flex-wrap items-center justify-between gap-3 mb-6">
        ...
        @else
            ...
            <div class="flex flex-wrap items-center gap-3" data-queue-filter-bar>
                {{-- 1. Date window — Show: All | 2 Days | Today Only (default
                     Today). Same DispatchDateRangeMode as the Dispatch and
                     Schedule boards: end-bound only, so overdue always shows;
                     2 Days = today + tomorrow (Queue Line's original prep
                     window); All = every upcoming eligible delivery.
                ...
                <div class="inline-flex items-center rounded-md border border-gray-300 overflow-hidden divide-x divide-gray-300" role="group" aria-label="Date Window">
                    ...
                    <button type="button" wire:click="$set('range', '2_days')" data-queue-range="2_days"
                        class="{{ $segBase }} {{ $range === '2_days' ? $segOn : $segOff }}">2 Days</button>
                    <button type="button" wire:click="$set('range', 'today')" data-queue-range="today"
                        class="{{ $segBase }} {{ $range === 'today' ? $segOn : $segOff }}">Today Only</button>
                </div>
