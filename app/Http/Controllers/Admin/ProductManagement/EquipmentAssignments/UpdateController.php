...
{
    public function __invoke(Request $request, ProductEquipmentAssignment $equipmentAssignment): RedirectResponse
    {
        $validated = $request->validate([
            'product_id' => [
                'required',
                'exists:products,id',
                Rule::unique('product_equipment_assignments', 'product_id')->ignore($equipmentAssignment->id),
            ],
        ...
        DB::transaction(function () use ($validated, $equipmentAssignment) {
        ...
        return redirect()
            ...
            ->with('success', 'Equipment assignment updated successfully.');
    }
