...
use App\Jobs\SendReturnDayBeforeRentalReminderJob;
 ...
 * at all (unlike SendDeliveryDayBeforeRentalReminderJob, which already had
 * one). Same fixture/harness pattern as SendDeliverySameDayRentalReminderJobTest:
 * TwilioService is constructed directly inside the job (not container-resolved),
 * so fake-but-well-formed encrypted credentials are seeded to let construction
 * succeed; the real Twilio API call then fails authentication (caught
 * internally) so no real SMS is ever sent. Assertions read the `sms_logs` row
 * TwilioService::logSmsAttempt() writes unconditionally after each attempt.
 */
class SendReturnDayBeforeRentalReminderJobTest extends TestCase
{
    use RefreshDatabase;
    ...
    protected function setUp(): void
    {
        ...
        $this->customer = Customer::create([
            ...
            'email' => 'return-day-before-job-test@example.com', 'status' => 'Active',
        ]);
        ...
        foreach ([
            'rental_return_day_before_store_message' => 'RETURN_DAY_BEFORE_STORE_MSG for {{customer_name}} by {{pickup_time}}',
            'rental_return_day_before_store_message_enabled' => '1',
            'rental_return_day_before_truck_message' => 'RETURN_DAY_BEFORE_TRUCK_MSG for {{customer_name}} by {{pickup_time}}',
            'rental_return_day_before_truck_message_enabled' => '1',
        ] as $name => $value) {
    ...
    private function makeRecord(string $orderNumber, string $pickupStatus = 'Pending', ?string $pickupTime = '09:00:00'): array
    {
        ...
        $orderProduct = OrderProduct::create([
            ...
            'pickup_date'            => Carbon::now('America/Chicago')->addDay()->toDateString(),
    ...
    {
        (new SendReturnDayBeforeRentalReminderJob())->handle();
    }

    public function test_sends_when_no_matching_sms_log_exists(): void
    {
        [$order] = $this->makeRecord('RDB-DEDUP-NOLOG');

        $this->runJob();

        $log = SMSLog::where('order_id', $order->id)->first();
        $this->assertNotNull($log);
        $this->assertSame(SmsType::RETURN_DAY_BEFORE, $log->sms_type);
        $this->assertStringContainsString('RETURN_DAY_BEFORE_STORE_MSG', $log->message);
    }

    public function test_does_not_resend_when_a_matching_sms_log_already_exists(): void
    {
        [$order] = $this->makeRecord('RDB-DEDUP-EXISTING');
        SMSLog::create([
    ...
    {
        [$order] = $this->makeRecord('RDB-DEDUP-OTHERTYPE');
        SMSLog::create([
        ...
        $this->assertTrue(
            SMSLog::where('order_id', $order->id)->where('sms_type', SmsType::RETURN_DAY_BEFORE->value)->exists()
        );
    }

    public function test_existing_date_and_pending_status_conditions_still_apply(): void
    {
        [$order] = $this->makeRecord('RDB-DEDUP-NOTPENDING', pickupStatus: 'Completed');
    ...
    public function test_pickup_date_not_tomorrow_is_still_excluded(): void
    {
        [$order, $orderProduct] = $this->makeRecord('RDB-DEDUP-WRONGDATE');
        $orderProduct->update(['pickup_date' => Carbon::now('America/Chicago')->toDateString()]);
    ...
    {
        [$order] = $this->makeRecord('RDB-WEEKEND', pickupTime: '14:00:00');
    ...
    {
        [$order] = $this->makeRecord('RDB-STANDARD', pickupTime: '09:00:00');
    ...
    {
        [$order] = $this->makeRecord('RDB-FORMAT', pickupTime: '16:30:00');
    ...
    {
        [$order] = $this->makeRecord('RDB-NOFALLBACK', pickupTime: '11:15:00');
    ...
    {
        [$order] = $this->makeRecord('RDB-NOPICKUPTIME', pickupTime: null);
