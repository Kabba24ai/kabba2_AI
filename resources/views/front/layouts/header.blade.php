...
<head>
    ...
    <title>{{ $metaTitle ?? 'Rent n King - Equipment Rentals' }}</title>
            ...
            <div class="max-w-7xl mx-auto py-1 flex items-center  px-[15px]">
                ...
                <div class="flex space-x-4 5/6 w-full items-center justify-end text-[#fff]">
                    <ul class="lg:flex hidden">
                        ...
                        <li class="px-[18px] group">
                            ...
                            <ul class=" absolute min-w-[120px] pt-[25px] pb-[20px] top-[60px] right-[15px] border-[1px] translate-y-[10px] bg-[#222] border-[#222] bg-opacity-90  opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 ease-in-out z-10 grid grid grid-flow-col grid-rows-[repeat(4,_auto)] gap-y-4 gap-x[0.5rem] before:content-[''] before:h-[30px] before:absolute before:top-[-30px] before:w-full ">
                                ...
                                <li class="{{ $category->childCategories->isNotEmpty() ? 'group/item relative' : 'flex w-full whitespace-nowrap' }}">
                                    <a href="{{ route('front.category.listing',$category->slug) }}" class="hover:text-yellow-400 px-[18px] text-[14px] transition-all duration-300 ease-in-out">
                                        {{ $category->title }}
                                    </a>
                        ...
                        <li class=" px-[18px]">
                            <a href="{{ route('front.login') }}" class="hover:text-yellow-400 text-[14px] transition-all duration-300 ease-in-out">Log In</a>
                        </li>
                    </ul>
                </div>
                <div>
                    ...
                    <div class="py-[15px] md:px-[15px] h-[calc(100vh-80px)] overflow-y-scroll">
                        <ul class="lg:hidden block text-center">
                          ...
                          grid gap-y-4">
                                    ...
                                    <li class="py-[10px] group relative">
                                        <a href="javascript:void(0)" class="hover:text-yellow-400 px-[18px] text-[14px] transition-all duration-300 ease-in-out">Equipment Rentals</a>
                                        <ul class="bg-[#222] text-center transition-all duration-300 ease-in-out z-10 grid gap-y-4">
                                            @foreach (\App\Helpers\CustomHelper::categoryTree() as $category)
                                            <li class="{{ $category->childCategories->isNotEmpty() ? 'group/item relative' : 'flex w-full whitespace-nowrap' }}">
                                                <a href="" class="hover:text-yellow-400 w-full px-[18px] text-[14px] transition-all duration-300 ease-in-out">
                                                    {{ $category->title }}
                                                </a>
                                                @if ($category->childCategories->isNotEmpty())
