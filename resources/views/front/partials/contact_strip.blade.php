{{-- ============================================================
     Shared Contact Strip — GLOBAL component
     One source of truth: Home Page Builder → Contact Strip
     (the home page's contact_strip section, via HomePageService,
     shared with every front view as $contactStripHp).
     Rendered on the homepage (floated over the hero bottom edge,
     $overlapHero = true) and on the Contact Us page (normal flow).
     Each column is controlled by its builder item status.
============================================================ --}}

@php
    $overlapHero = $overlapHero ?? false;
    $strip      = $contactStripHp ?? null;
    $phoneCard  = $strip?->phoneCard;
    $storeCard1 = $strip->storeCard1 ?? null;
    $storeCard2 = $strip->storeCard2 ?? null;
    $searchCard = $strip?->searchCard;
...
<section id="contact-strip" class="relative z-20 pb-2 md:pb-0 mb-5">
    <div class="container mx-auto px-4 md:px-6 lg:px-8
                {{ $overlapHero ? '-mt-12 md:-mt-16 lg:-mt-20' : 'mt-2' }}">
        <div class="text-center uppercase text-[#171636] mt-6 mb-6">
            @if(!empty($strip?->subtitle))
