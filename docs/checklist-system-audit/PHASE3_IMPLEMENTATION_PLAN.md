...
**Revision note (2026-07-17, later same day, after P3-12A):** **P3-12B resolved — BUG-12.** (Renamed from the plan's original "P3-12a" label to avoid collision with P3-12A, which resolved BUG-3.) `SaveDeliveryController` never cross-checked the delivered equipment's category against the booked product's category. Before implementing, verifying the actual schema surfaced a real ambiguity the original audit finding didn't anticipate: `OrderProduct` has no category column, and the real path (`Product::categories()`) is many-to-many, not the single-value comparison assumed — resolved via explicit confirmation (membership check; zero-category products treated as unknown, not logged) rather than guessed. Implemented exactly per the item's own recommended observe-first approach: a non-blocking warning log mirroring PR-A4's existing pattern, added to `SaveDeliveryController` only (42 lines, one file) — no architecture, API, mobile, schema, or admin-controller changes. Two new tests added to the existing PR-A4 observability suite; verified via revert-and-reproduce that they actually catch the bug. Full CustomerChecklists regression green (51 passed, 235 assertions). Extending the same check to admin equipment-assignment controllers (e.g. `AssignEquipmentController`) was explicitly deferred, not fixed, per this PR's single-controller scope. See `P3_12B_BUG12_EQUIPMENT_CATEGORY_MISMATCH_LOG.md`.
...
**BUG-12 — No validation that delivered equipment belongs to the order product's category** — **Medium** — ✅ **RESOLVED (2026-07-17, P3-12B)**
...
- **Previous behavior:** Any equipment record can be attached to any order product's delivery, regardless of category alignment.
- **Data-model discovery (resolved before implementation, not guessed):** `OrderProduct` has no category column; the actual path is `OrderProduct → Product → Product::categories()`, a many-to-many relation (via `ProductCategoryChild`) — not the single-value comparison the original finding assumed, since `Equipment::product_category_id` is a single `belongsTo`. Resolved as a membership check (equipment's one category must be among the product's assigned categories); products with zero assigned categories are treated as unknown and intentionally not logged, to avoid noise from incomplete category data.
- **Resolution:** Implemented exactly per this entry's own recommended approach — an observe-first warning log (mirroring PR-A4's `logIfDeliveryIncomplete` pattern) in `SaveDeliveryController`, added alongside the existing PR-A4 log call. No blocking, no response/status change. See `P3_12B_BUG12_EQUIPMENT_CATEGORY_MISMATCH_LOG.md` for full detail including the category-ambiguity resolution.
- **Desired behavior:** Achieved for this PR's scope — mismatches are now measurable via the `api_errors` log channel; hard rejection remains a future decision pending real-world frequency data, per this entry's own original recommendation.
- **Technical impact:** Contained to `SaveDeliveryController` only for this PR (42 lines, one file). Admin equipment-assignment controllers (e.g. `AssignEquipmentController`) were explicitly **not** extended in this pass — deferred, not silently skipped (see the doc's "Deferred findings").
- **Dependencies:** Same investigation/code area as **BUG-2**/**BUG-4** (delivery-time controllers) — sequenced after them as planned. Also relevant context for **ARCH-1**'s eventual feasibility study, since category-matching logic would need to be designed once, not per-tree, if the Rental Ready/Customer Admin trees are ever unified.
- **Files changed:** `app/Http/Controllers/Api/Admin/V1/Orders/CustomerChecklists/SaveDeliveryController.php`; `tests/Feature/CustomerChecklists/CompletenessObservabilityLoggingTest.php` (2 new tests).
- **Estimated effort:** Small-Medium (2-3 days) — matched actual effort.
- **Priority:** Medium — real business risk, but no confirmed live incident found in any of the reviewed audit/runtime-validation documents; treated as "close the gap," not "emergency fix," consistent with the observe-first resolution.
...
| 12a *(newly added — inserted here to preserve items 13-20's existing numbering)* | **P3-12B** — Bug fix *(renamed from P3-12a to avoid collision with P3-12A, which resolved BUG-3 — see revision note below)* | BUG-12 (equipment-category cross-check) | Same delivery-controller investigation pass as items 9-10 (BUG-2/BUG-4); independently shippable — ✅ **RESOLVED** |
