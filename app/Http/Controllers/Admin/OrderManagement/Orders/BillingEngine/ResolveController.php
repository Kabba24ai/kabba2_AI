        ...
        $closed = DB::transaction(function () use ($request, $chargeUniqueId) {
            ...
            if ($charge->legacyCustomerAccount) {
                ...
                if ($ca->fuel_alert_status === 'pending') {
                    AlertLifecycleService::transitionCustomerAccount((int) $ca->id, 'fuel', 'resolved', $by);
                } elseif ($ca->damage_alert_status === 'pending') {
                    AlertLifecycleService::transitionCustomerAccount((int) $ca->id, 'damage', 'resolved', $by);
                }
            }
            ...
            if ($charge->order_product_id && ! $charge->customer_account_id) {
                ...
                AlertLifecycleService::transitionOrderProduct((int) $charge->order_product_id, $alertType, 'resolved', $by);
            }
