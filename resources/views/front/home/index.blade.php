...
@section('content')
    ...
    <div class="container">
      <form action="#">
        <div class="relative z-0 w-full md:w-96 mr-auto ml-auto my-6 group flex shadow-md rounded-lg">
          <input type="text" placeholder="" class="block bg-light py-3 px-4 w-full rounded-l-lg text-sm text-gray-900 border-0 appearance-none focus:border-b focus:outline-none focus:ring-0 focus:border-yellow-400 peer" required="">
          <label for="name" class="pointer-events-none  z-1 px-6 absolute text-base rounded-l-lg text-gray-500 duration-300 transform -translate-y-8 scale-75 top-2 origin-[0] peer-placeholder-shown:scale-100 peer-placeholder-shown:translate-y-0 peer-focus:scale-75 peer-focus:-translate-y-8 -translate-x-2.5 peer-focus:text-black">
            Search for Products
          </label>
          <button class="border-0 bg-yellow-500 items-center font-medium flex px-6 py-1 leading-4 rounded-r-lg text-sm hover:bg-yellow-400  transition-all duration-500 ease-in-out">
                        ...
                        <a href="{{ route('front.categories.index', $category->slug) }}"
                            ...
                            <div class="w-full">
                                <img src="{{ $category->media?->url }}" alt="Skid Steer"
                                    class="mx-auto category-product object-contain rounded-md object-cover h-[auto]" />
                            </div>
                            <div>
                                <h3
