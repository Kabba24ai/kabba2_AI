                        ...
                        <div class="bg-white rounded-md shadow border p-4">
                            <div class="flex justify-between items-start">
                                <div class="flex gap-3 items-start">
                                    ...
                                    <div>
                                        <h2 class="text-base font-semibold text-gray-800">{{ $user->first_name }}
                                            {{ $user->middle_name }} {{ $user->last_name }} </h2>
                                        <div class="flex gap-2 mt-1 text-xs">
                                            {{-- Status Badge --}}
                                            @if ($user->status === 'Active')
                                                <span
                                                    class="bg-green-100 text-green-800 px-2 py-0.5 rounded-full">active</span>
                                            @elseif($user->status === 'Inactive')
                                                <span
                                                    class="bg-red-100 text-red-800 px-2 py-0.5 rounded-full">inactive</span>
                                            @endif
                                ...
                                <div class="flex gap-2">
                                    <a href="{{ route('admin.hrm.users.view', $user->unique_id) }}"><x-heroicon-o-eye
                                            class="w-4 h-4 text-grey-600" /></a>
                            ...
                            <div class="mt-4 text-sm border-b text-gray-700 space-y-2 mb-4">
                                ...
                                <div class="flex items-center gap-2">
                                    ...
                                    {{ \App\Helpers\CustomHelper::formatPhone($user->mobile_phone ?? '') ?: ' ' }} <span
                                        class="text-gray-400 mx-1">-</span> <span
                                        class="text-gray-400">{{ \App\Helpers\CustomHelper::formatPhone($user->phone_number ?? '') ?: ' ' }}</span>
                                </div>
                                <div class="flex items-center gap-2">
                                    <x-heroicon-o-map-pin class="w-4 h-4 text-gray-900" />
                                    {{ $user->street_address }} {{ $user->city }}
                                    {{ optional($user->stateRelation)->name }} {{ $user->zip_code }}
                                    {{ $user->country }}
                                </div>
                                <div class="flex items-center gap-2">
