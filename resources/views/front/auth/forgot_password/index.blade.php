                ...
                <main class="flex items-center justify-center p-2 md:p-4 lg:p-8 bg-white shadow rounded">
                    <div class="border-dotted border-4 border-gray w-full p-2 md:p-4 lg:p-8">
                        ...
                        {{ html()->form()->attributes([
                                    'class' => 'space-y-4',
                                    'method' => 'POST',
                                     'action' => route('front.auth.forgot-password.send.token'),
                        ...
                        <div class="relative z-0 w-full my-6 group">
                            {{ html()->email('email', old('email'))->attributes([
                                    ...
                                    'data-parsley-remote' => route('front.auth.forgot-password.check.customer'),
                                    'data-parsley-remote-validator' => 'customerexists',
                                    'data-parsley-remote-message' => 'Customer with this email does not exist.',
                                ])->placeholder('') }}
                        ...
                        <div class="w-full mt-5 flex justify-center">
                            <button type="submit" id="checkEmailBtn" class="border-0 bg-yellow-400 font-medium flex px-6 py-3 items-center leading-4 rounded-lg text-sm hover:bg-yellow-300 transition-all duration-500 ease-in-out">
...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        window.Parsley.addAsyncValidator('customerexists', function(xhr) {
        ...
        emailField.addEventListener('input', debounce(() => {
            const parsley = $(emailField).parsley();
    ...
    //         fetch("{{ route('front.auth.forgot-password.send.token') }}", {
    //                 method: "POST",
    //                 headers: {
