...
> **BRANCH NOTE — `feature/gift-cards-release`**
>
> The company-wide reporting treatment described below is **NOT present on this
> branch**. It depends on `CollectedRevenueQuery` / `ProportionalPaymentSplit`,
> which do not exist on `raj_development`. No shared reporting file is modified
> here. See [`RELEASE_NOTE_REVIEW_BRANCH.md`](RELEASE_NOTE_REVIEW_BRANCH.md) for
> what that means in practice.
>
> Everything else in this document — the accounting model, schema, service
> behaviour, concurrency, permissions — is accurate for this branch.
