...
**BUG-2 — Re-delivering with the SAME equipment never calls `markRented()` again (N1)** — **High**
...
- **Current behavior:** Confirmed via `PHASE3_RESULTS.md` N1 — order product shows `delivery_status='Completed'` (freshly delivered) while its equipment `current_status` remains `'damaged'` from the prior return — a state combination the system's own logic shouldn't allow to coexist.
- **Desired behavior:** Any successful delivery submission calls `EquipmentStatusService::markRented()` regardless of whether the equipment matches the previous assignment.
- **Business impact:** Equipment can appear "delivered" to a customer while still flagged damaged in the system — a dispatch/safety-adjacent inconsistency.
- **Technical impact:** Small, contained fix in `SaveDeliveryController`'s branch condition.
- **Dependencies:** Should be designed together with **BUG-4** (same controller area) and requires the characterization-tests-first discipline (see §2.1 Wave 3 below), since zero test coverage exists for this controller's branch logic today.
- **Recommended solution:** Broaden the branch condition to always call `markRented()` on a successful save, or add a dedicated explicit "same-equipment re-delivery" path that still transitions status.
...
**BUG-3 — Soft-deleted checklist questions leave live child answer rows behind (N2)** — **Medium**
...
- **Current behavior:** Confirmed via `PHASE3_RESULTS.md` N2 — after a re-delivery soft-deletes the old question row, its child answer rows remain fully live (`deleted_at IS NULL`) with `is_return_answer=1` still set, orphaned under a parent no longer in the active checklist.
- **Desired behavior:** Deleting/soft-deleting a checklist question also soft-deletes its answer rows.
- **Business impact:** Low direct impact (no confirmed downstream consumer reads orphaned rows today), but a latent data-integrity risk for any future reporting/audit feature.
- **Technical impact:** Add a model `deleting` hook (mirrors the pattern already used for `RentalReadyChecklistQuestion`/`CustomerAdminQuestion`'s `templateQuestions()->delete()` cleanup in PR-B4.2).
- **Dependencies:** None.
- **Recommended solution:** Add a `deleting` boot hook cascading to answer rows; add regression test proving no orphans remain after a re-delivery cycle.
- **Estimated effort:** Small (1-2 days).
- **Risk:** Low.

---

**BUG-4 — No re-delivery guard equivalent to save-return's 409 duplicate-signature guard** — **Medium**
...
- **Current behavior:** A naive client retry (or a genuine but unintended re-delivery) silently destroys and rebuilds the prior checklist snapshot, re-uploads the signature, and re-fires all side effects.
- **Desired behavior:** Delivery gets the same kind of guard save-return already has, tuned to delivery's semantics.
- **Business impact:** Data-loss risk if a return already happened in the gap between two delivery submissions.
- **Technical impact:** Contained to `SaveDeliveryController`.
- **Dependencies:** Sequence with **BUG-2** (same file, same investigation pass).
- **Recommended solution:** Add a guard mirroring `SaveReturnController`'s pattern; add regression test.
- **Estimated effort:** Small-Medium (2 days).
- **Risk:** Low-Medium — must confirm this doesn't block a legitimate current workflow (e.g., staff correcting a delivery mistake) before hardening; consider observe-first logging if any doubt.

---

**BUG-5 — `RemoveController` only reverts delivery-side fields** — **Medium**
- **Root cause:** The checklist-removal controller resets only `delivery_*`/`is_delivered`-family fields, leaving `pickup_*`/`is_returned` untouched if a product was already returned.
- **Current behavior:** An inconsistent partial revert — a removed/undone delivery can leave return-side state stale and contradictory.
- **Desired behavior:** Removal either reverts both sides consistently or explicitly refuses when a return has already occurred (with a clear error), rather than silently leaving mixed state.
- **Business impact:** Confusing state for staff investigating an order's history; possible downstream reporting inconsistency.
- **Technical impact:** Contained to `RemoveController`.
- **Dependencies:** None.
- **Recommended solution:** Decide (product input needed) whether removal should cascade to return-side fields or reject when a return exists; implement accordingly with regression tests for both cases.
- **Estimated effort:** Small-Medium (2-3 days incl. tests).
- **Risk:** Low technically; needs a product decision on desired behavior before implementation (mirrors this project's decision-matrix discipline).
