...
// Event
use App\Events\Admin\ProductManagement\Categories\Action\InactiveEvent;
        ...
        if ($request->has('selected_ids') && is_array($request->get('selected_ids')) && count($request->get('selected_ids')) > 0) {
            ...
            if (!is_null($product_category_list)) {
                ...
                foreach ($product_category_list as $product_category_item) {
                    $product_category_item->status = 'Inactive';
                ...
                if (is_array($changesArr) && count($changesArr) > 0) {
                    InactiveEvent::dispatch([
                        'user_item' => $user_item,
                        'item_list' => $item_list,
                        'changes' => $changesArr
                    ]);
                }
                flash('Selected items successfully inactivated.')->success();
                return response([], 200);
            }
        }
    }
