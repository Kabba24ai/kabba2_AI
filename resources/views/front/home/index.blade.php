...
<script>
    document.addEventListener("DOMContentLoaded", () => {
        ...
        input.addEventListener("keyup", async function() {
            ...
            try {
                const response = await fetch("{{ route('front.home.search.autocomplete') }}?query=" + encodeURIComponent(query));
                ...
                if (data.products.length > 0) {
                    html += `<div class='px-3 py-2 font-semibold text-gray-600 border-b bg-gray-50'>Products</div>`;
