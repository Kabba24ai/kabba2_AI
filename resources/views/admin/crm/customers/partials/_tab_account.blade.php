...
<input type="hidden" id="existing_tags_json" value='{{ $customer->tag_objects }}'>
            ...
            <div class="flex flex-wrap items-center gap-2">
                ...
                @endphp
                    ...
                    <button onclick="confirmAndActive({{ $customer->id }}, '{{ $account['status'] }}')" type="button"
                        ...
                        Account</button>
                @endif
                <button type="button" id="openResetPasswordModal"
                    class="bg-yellow-500 text-white px-6 py-3 rounded-lg text-md static-view">Reset Password</button>
                <button id="editBtn" type="button"
                    class="bg-blue-600 inline-flex items-center px-6 py-3 text-white text-md font-medium rounded-lg hover:bg-green-700 transition">
                    Edit Information</button>
                <button type="submit" id="saveBtn" style="display:none;"
                    class="saveBtn bg-green-600 text-white px-6 py-3 rounded-lg text-md">Save Changes</button>
                <button type="button" id="cancelBtn" style="display:none;"
                    class="bg-gray-700 text-white px-6 py-3 rounded-lg text-md">Cancel</button>
            </div>
        </div>
    </div>
</div>
        ...
        <div class="grid grid-cols-1 gap-4 text-sm text-gray-700">
            ...
            <div class="col-span-2">
                ...
                <div class="flex gap-2 mb-4">
                    ...
                    {{-- Website Name --}}
                    {!! html()->text('company_website', old('company_website', $company_website ?? null))->class([
                        ...
                        ])->placeholder('example')->id('company_website') !!}
    ...
    @foreach ($defaultAddresses as $index => $addressItem)
        ...
        <div class="address-block bg-white rounded-xl shadow-sm p-5 border border-gray-200"
            ...
            <input type="hidden" name="addresses[{{ $index }}][address_id]" value="{{ $addresse->id ?? '' }}"
                class="address_id">
            <input type="hidden" name="addresses[{{ $index }}][type]" value="{{ $addressItem['label'] }}"
                class="type">
            <input type="hidden" name="addresses[{{ $index }}][is_primary]"
                value="{{ $addresse?->is_primary ? 1 : 0 }}" class="is_primary_input">
            ...
            <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-4 gap-3">
                ...
                @if ($addressItem['label'] == 'Shipping')
                    <!-- Checkbox -->
                    <label
                        class="edit-view flex items-center gap-2 text-sm text-gray-700 cursor-pointer font-semibold">
            ...
            <div class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="edit-view">
                        <label class="text-xs text-gray-500 font-medium ">First Name </label>
                        {!! html()->text("addresses[$index][first_name]", old("addresses.$index.first_name", $addresse->first_name ?? $customer->first_name ?? ''))->class([
                                'edit-view pl-2 pr-2 px-3 py-3 w-full border rounded-md text-sm border-gray-300',
                    ...
                    <div class="edit-view">
                        <label class="text-xs text-gray-500 font-medium ">Last Name </label>
                        {!! html()->text("addresses[$index][last_name]", old("addresses.$index.last_name", $addresse->last_name ?? $customer->last_name ?? ''))->class([
                                'edit-view pl-2 pr-2 px-3 py-3 w-full border rounded-md text-sm border-gray-300',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div class="edit-view">
                        ...
                        {!! html()->text("addresses[$index][city]", old("addresses.$index.city", $addresse->city ?? ''))->class([
                                'edit-view pl-2 pr-2 px-3 py-3 w-full border rounded-md text-sm border-gray-300',
                    ...
                    <div class="edit-view">
                        ...
                        {!! html()->text("addresses[$index][zip_code]", old("addresses.$index.zip_code", $addresse->zip_code ?? ''))->class([
                                'edit-view pl-2 pr-2 px-3 py-3 w-full border rounded-md text-sm border-gray-300',
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div class="edit-view">
                        ...
                        {!! html()->select(
                            ...
                            )->class('pl-2 pr-2 px-3 py-3 w-full border border-gray-300 rounded-md text-sm Country')->attributes([
                                'autocomplete' => 'off',
                            ]) !!}
                    ...
                    <div class="edit-view">
                        ...
                        {!! html()->text("addresses[$index][phone]", old("addresses.$index.phone", $addresse->phone ?? $customer->phone ?? ''))->class([
                                'masked-phone edit-view pl-2 pr-2 px-3 py-3 w-full border rounded-md text-sm border-gray-300',
                                'border-red-500' => $errors->has('phone'),
                            ])->attributes([
                                'maxlength' => 14,
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6 items-stretch">
    ...
    <div class=" flex flex-col h-full">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 flex-grow">
            ...
            <!-- Tags -->
            <div class="static-view bg-white rounded-lg shadow border border-gray-200 p-6 flex flex-wrap gap-2 "
                id="customerTagsWrapperacount">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="space-y-2 text-sm text-gray-700">
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="edit-view">
                    <input
                        ...
                        type="text" name="tax_document_valid_until" id="tax_document_valid_until"
            ...
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="static-view">
                    <span
                        class="text-sm">{{ App\Helpers\CustomHelper::formatDate($customer->tax_document_upload_date) ?? 'N/A' }}</span>
                </div>
                <div class="edit-view">
                    <input
                        class="w-full sm:w-32 md:w-32 border rounded-md px-3 py-3 text-sm shadow-sm focus:outline-none focus:ring focus:border-blue-500 bg-white datepicker border-gray-300"
                        ...
                        name="tax_document_upload_date" id="tax_document_upload_date" placeholder="MM-DD-YYYY"
                        autocomplete="off" />
                </div>
            </div>
        </div>
...
<div id="taxdocModalWrapper" style="display: none;"
    ...
    <div class="modal-scrollable w-full mx-auto">
        <div
            ...
            <div class="flex justify-between items-center px-6 pt-4">
                ...
                <button type="button" id="closetaxdocBtn"
            ...
            <div class=" p-6 overflow-y-auto">
                <div class="max-w-md mx-auto">
                    ...
                    {{-- Open Form --}}
                    {!! html()->form()->id('taxDocForm')->attribute('enctype', 'multipart/form-data')->attribute('autocomplete', 'off')->attribute('data-parsley-validate', true)->class('space-y-8')->open() !!}
                    ...
                    <div class="border-2 border-dashed border-gray-300 p-6 text-center rounded">
                        <div id="uploadUI" class="flex flex-col items-center justify-center">
                            ...
                            <label id="chooseFileLabel" class="mt-3 inline-block cursor-pointer">
                                <input type="file" id="tax_document" name="tax_document" class="hidden"
                                    accept=".pdf,.png,.jpg,.jpeg" onchange="handleFileChange(event)" required>
                        ...
                        <div id="fileActions"
                            class="hidden text-sm mt-3 text-gray-700 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 sm:gap-4">
                            <!-- File Name -->
                    ...
                    <div class="mt-5 flex justify-end gap-2">
                        <button type="button" id="cancelBtntaxdoc"
                            class="px-6 py-3 border border-gray-300 rounded-lg text-md">Cancel</button>
                        <button type="submit"
                            class="saveBtntax px-6 py-3 text-md rounded-lg bg-teal-600 text-white hover:bg-teal-700">Upload
                            Document</button>
                    </div>
...
<!-- Hidden Delete Form -->
<form id="delete-media-form-{{ $customer->id }}" method="POST"
    action="{{ route('admin.crm.customers.tax-document.delete', $customer->unique_id) }}" style="display: none;">
    @csrf
    @method('DELETE')
</form>
<form id="suspend-customer-form-{{ $customer->id }}" method="POST"
    action="{{ route('admin.crm.customers.status-update.customer', $customer->unique_id) }}" style="display: none;">
...
<div id="notesModal" class="fixed inset-0 z-[99999] flex items-center justify-center bg-black/50 px-4 py-10 hidden">
    <div class="modal-scrollable w-full mx-auto">
        <div
            class="bg-white rounded-lg shadow-xl w-full mx-auto max-w-sm flex flex-col max-h-full overflow-hidden border border-gray-200">
            ...
            <div class="px-6 overflow-y-auto max-h-[70vh] mt-5 mb-5">
                <div class="mb-4">
                    ...
                    {!! html()->select('userSelect', $employees->pluck('full_name', 'id')->toArray())->id('userSelect')->class([
            ...
            <div class="flex justify-end gap-2 pt-4 pb-4 px-4 border-t border-gray-200">
                <button type="button" onclick="closeNotesModal()"
                    class="px-6 py-3 text-md rounded-lg border border-gray-300 bg-white">Close</button>
                <button type="button" id="saveNoteBtn"
                    class="px-6 py-3 text-md rounded-lg border border-grey-300 bg-blue-600 text-white">Save</button>
            </div>
        </div>
    </div>
</div>
                        ...
                        'text-yellow-600'
                                ...
                                if (status === 'Approved') {
                                    document.querySelectorAll(
                                        `[data-customer-id="${customerId}"].approve-btn`).forEach(
                                        btn => btn.style.display = 'none');
                                    document.querySelectorAll(
                                        `[data-customer-id="${customerId}"].reject-btn`).forEach(
                                        btn => btn.style.display = 'none');
                                }
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            ...
            const updateUrlTemplate =
                `{{ route('admin.crm.customers.notes.update', ['note' => 'NOTE_ID_PLACEHOLDER']) }}`;
            const deleteUrlTemplate =
                `{{ route('admin.crm.customers.notes.delete', ['note' => 'NOTE_ID_PLACEHOLDER']) }}`;
