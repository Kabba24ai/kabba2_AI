...
@push('js')
  ...
  <script>
      document.addEventListener('DOMContentLoaded', function() {
          ...
          /* -------------------------------------
            Load existing selections (Edit mode)
          ------------------------------------- */
          ...
          if (selectedProducts.length > 0) {
              ...
              // Disable all checkboxes from other categories
              checkboxes.forEach(cb => {
          ...
          /* -------------------------------------
               Multiple selection (same category)
          ------------------------------------- */
          ...
          function filterProducts() {
              ...
              const selectedCategoryFilter = categoryDropdown.value;
              ...
              productList.querySelectorAll('div.mb-3').forEach(categoryBlock => {
                  ...
                  const categoryVisible = !selectedCategoryFilter || selectedCategoryFilter === categoryId;
                  ...
                  categoryBlock.querySelectorAll('li').forEach(productItem => {
                      ...
                      const show = matches && categoryVisible;

                      productItem.style.display = show ? 'flex' : 'none';
                      if (show) hasVisibleProduct = true;
                  });
          ...
          // Initialize category IDs for blocks

          productList.querySelectorAll('div.mb-3').forEach((block) => {
              const title = block.querySelector('h3').textContent.trim();
              const matchedOption = Array.from(categoryDropdown.options)
                  .find(opt => opt.text.trim() === title);
              if (matchedOption) {
                  block.setAttribute('data-category-id', matchedOption.value);
