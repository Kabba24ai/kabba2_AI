...
**BUG-3 — Soft-deleted checklist questions leave live child answer rows behind (N2)** — **Medium** — ⚠️ **PARTIALLY RESOLVED (2026-07-17, P3-12) — see scope note**
...
- **Resolution (RemoveController only):** P3-12 fixed this cascade inside `RemoveController::__invoke()` — the checklist-removal path — by soft-deleting every child answer row (looked up from the already eager-loaded `checklistQuestions.answers` relation) immediately before the existing bulk question soft-delete, inside the same transaction. This mirrors an already-correct, pre-existing precedent in `OrderProduct`'s own `deleting`/`restoring` model hooks, which already cascade this exact relationship correctly.
- **Scope note — the original N2 finding is NOT fully closed:** the audit's own root-cause description above ("after a **re-delivery** soft-deletes the old question row") actually points at `SaveDeliveryController`'s destructive-rebuild statement, not `RemoveController` — and P3-12 was explicitly scoped to `RemoveController` only. Four other call sites of `checklistQuestions()->delete()` with the identical uncascaded gap were found during P3-12's audit and deliberately left unfixed, per that PR's explicit scope: `AssignEquipmentController.php:71`, `RemoveEquipmentController.php:51`, `UpdateProductScheduleController.php:209` and `:250`, and `SaveDeliveryController.php:121` itself (the literal scenario N2 originally described). See `P3_12_BUG3_SOFT_DELETE_CHECKLIST_ANSWERS.md`'s "Additional orphan scenarios found, NOT fixed" section — these remain open backlog items, candidates for a follow-up item (e.g. **BUG-3b** or folded into a future pass) if a shared helper/trait is judged worthwhile once all five sites are considered together.
- **Business impact:** Low direct impact (no confirmed downstream consumer reads orphaned rows today), but a latent data-integrity risk for any future reporting/audit feature — now closed for checklist removal, still open for the other four sites listed above.
- **Technical impact:** Contained to `RemoveController` for this PR — one new import, one new two-line statement.
- **Files changed:** `app/Http/Controllers/Api/Admin/V1/Orders/CustomerChecklists/RemoveController.php`; `tests/Feature/CustomerChecklists/RemoveControllerCharacterizationTest.php` (BUG-3 test flipped, 2 new tests).
- **Estimated effort:** Small (1-2 days) — matched actual effort for the `RemoveController` scope.
- **Risk:** Low — verified via 71 passed / 254 assertions across the full CustomerChecklists/equipment-status regression surface, including a dedicated cross-order-product isolation test.
