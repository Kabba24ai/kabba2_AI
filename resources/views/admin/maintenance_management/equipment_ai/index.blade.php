    ...
    @if ($selectedCategory)
        ...
        <div x-data="{ activeTab: '{{ request('tab', 'profiles') }}' }">
            ...
            <div x-show="activeTab === 'comparison'"
                 x-data="{ showAddKeyForm: false, editingKeyId: null }"
                 x-cloak>
                ...
                <div x-show="showAddKeyForm" x-transition x-cloak
                    ...
                    <div class="border-b border-indigo-100 px-6 py-4 dark:border-indigo-700/30 flex items-center justify-between">
                        <div>
                    ...
                    <form method="POST"
                        ...
                        <div>
                            ...
                            <input type="text" name="spec_key" placeholder="platform_height_ft" value="{{ old('spec_key') }}"
                        ...
                        <div>
                            ...
                            <input type="text" name="display_label" placeholder="Platform Height" value="{{ old('display_label') }}"
                                class="w-full rounded-lg border border-gray-300 px-3 py-2 text-sm focus:border-brand-500 focus:outline-none focus:ring-1 focus:ring-brand-500 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                        </div>
                ...
                <div class="rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
                    <div class="border-b border-gray-100 px-6 py-4 dark:border-gray-700 flex items-center justify-between">
                        <div>
                            <h2 class="text-base font-semibold text-gray-900 dark:text-white">
                                Key Comparison Criteria
                                <span class="ml-1 text-sm font-normal text-gray-400">({{ $comparisonKeys->count() }})</span>
                            </h2>
                            <p class="mt-0.5 text-xs text-gray-500 dark:text-gray-400">
                                These specs are used to score substitution compatibility across all {{ $selectedCategory->title }} in this category.
                            </p>
                        </div>
                    ...
                    <div class="overflow-x-auto">
                        <table class="min-w-full divide-y divide-gray-100 text-sm dark:divide-gray-700">
                            <thead class="bg-gray-50 dark:bg-gray-800">
                                <tr>
                                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400 w-16">Order</th>
                                    ...
                                    <th class="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Comparison Type</th>
                            ...
                            <tbody class="divide-y divide-gray-100 bg-white dark:divide-gray-700 dark:bg-gray-900">
                                @forelse ($comparisonKeys as $ck)
                                    ...
                                    <tr x-show="editingKeyId !== {{ $ck->id }}"
                                        ...
                                        <td class="px-5 py-3 text-gray-500 dark:text-gray-400 tabular-nums">{{ $ck->sort_order }}</td>
                                        <td class="px-5 py-3">
                                        ...
                                        <td class="px-5 py-3 text-xs text-gray-600 dark:text-gray-400">{{ $ck->comparisonTypeLabel() }}</td>
                                        ...
                                        <td colspan="7" class="px-5 py-4">
                                            <form method="POST"
                                                ...
                                                <div>
                                                    ...
                                                    <input type="text" name="display_label" value="{{ $ck->display_label }}" required
                                                ...
                                                <div>
                                                    ...
                                                    <select name="importance_level"
                                                        ...
                                                        @foreach (['critical','high','medium','low'] as $lvl)
                                                            <option value="{{ $lvl }}" {{ $ck->importance_level == $lvl ? 'selected' : '' }}>{{ ucfirst($lvl) }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div>
                                                    ...
                                                    <select name="comparison_type"
                                                        ...
                                                        @foreach (['higher_is_better','lower_is_better','must_match','range_acceptable','informational_only'] as $ct)
                                                            <option value="{{ $ct }}" {{ $ck->comparison_type == $ct ? 'selected' : '' }}>{{ ucwords(str_replace('_',' ',$ct)) }}</option>
                                                        @endforeach
                                                    </select>
                                                </div>
                                                <div>
                                                    <label class="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1">Sort Order</label>
                                                    <input type="number" name="sort_order" value="{{ $ck->sort_order }}" min="0"
                                                        class="w-full rounded-md border border-gray-300 px-2 py-1.5 text-xs focus:border-brand-500 focus:outline-none dark:border-gray-600 dark:bg-gray-800 dark:text-gray-100">
                                                </div>
                                                <div class="flex items-center pt-4">
                                                    <label class="flex items-center gap-2 text-xs text-gray-700 dark:text-gray-300 cursor-pointer">
                                                        <input type="checkbox" name="is_required" value="1" {{ $ck->is_required ? 'checked' : '' }}
                                ...
                                @empty
                                    <tr>
                                        <td colspan="7" class="px-6 py-12 text-center text-gray-400 dark:text-gray-500">
                                            <x-heroicon-o-adjustments-horizontal class="mx-auto h-10 w-10 text-gray-300 dark:text-gray-600 mb-3" />
                                            <p class="font-medium">No comparison keys defined yet for {{ $selectedCategory->title }}.</p>
                                            <p class="mt-1 text-xs">
                                                Check the <strong>Key Comparison</strong> checkbox on any spec row in a Make / Model, or click <strong>Add Key</strong> above to define manually.
                                            </p>
                                        </td>
                                    </tr>
                                @endforelse
    ...
    @else
        ...
        <div class="rounded-2xl border border-gray-100 bg-white shadow-sm dark:border-gray-700 dark:bg-gray-900">
            <div class="flex flex-col items-center justify-center px-6 py-16 text-center">
                ...
                <p class="mt-3 text-sm font-medium text-gray-500 dark:text-gray-400">Select a category above to view AI profiles and comparison criteria.</p>
